...
- Full set: `enum/docs/figures/ruczinski_all96_topology.{svg,png}` — all 96
  four-strand motifs, the 48 Fig-1 panels highlighted. Generated by
  **`enum/scripts/render_all96.py`** (reads `ruczinski/queries/motifs.tsv`, calls
  `render()` per row, tiles into an 8×12 grid). It feeds the as-drawn
  `panel_strand_order`/`panel_ori_drawn` for the 48 panels (so they reproduce the
  2002 figure) and the canonical `spatial_seq`/`orientation` for the other 48.
  Motifs are embedded as `<g transform>` groups, NOT nested `<svg>` viewports —
  ImageMagick mis-renders the latter.
...
## Reproduce
```
python enum/scripts/render_motif_svg.py <outdir>     # 4-strand smoke set (4 motifs)
python enum/scripts/render_all96.py                  # full 96-motif grid -> figures/
python enum/scripts/render_s5_iso.py   <outdir>      # S5 iso smoke set
```
Raster preview (ImageMagick can't resolve currentColor):
`sed 's/currentColor/black/g' figures/ruczinski_all96_topology.svg | convert -background white -density 110 - out.png`
